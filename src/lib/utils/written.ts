 ...
 */
export const enclose = (a: string, b: string, c?: string): string => `${a}${b}${c || a}`
...
export const cleanJoin = function (arr: Array<string | null>, glue = ""): string {
  return arr.filter((string) => string && typeof string === "string").join(glue)
}
...
export const hyphenCase = (str: string, leading?: boolean): string =>
  (leading ? "-" : "") + cleanJoin(str.split(caseSplitter), "-").toLowerCase()
 ...
 */
export const humanCase = (str: string): string => cleanJoin(str.split(caseSplitter), " ")
...
export const wrapInTag = (
  ...
  const attrs = Object.entries(attributes).map(([key, val]) => {
 ...
 */
export const prettyList = (arr: unknown[], max?: number, opts: PrettyListOptions = {}): string => {
  ...
  if (opts.wrap) {
    arr = arr.map((s) => wrapInTag(s as string, opts.wrap))
  }
  ...
  if (max && max < len) {
    ...
    arr = arr.slice(0, max).concat(`${diffStr} ${more}`)
  }
...
export const hyphenate = (str: string, n = 10, softHyphen = "\u00AD"): string =>
  str.replace(/(^|[^>])+(?=$|\<)/g, (sub) =>
    sub.replace(new RegExp(`(\\w{${n - 1}})(\\w)`, "g"), (_w, a, b) => a + softHyphen + b)
  )
...
// quotes
type Quotes = "s" | "single" | "a" | "angle" | "g" | "guillemets" | "!" | "?" | "q" | string
 ...
 */
export const ordinal = (n: number, opts: OrdinalOptions = {}, gender = "m"): string => {
...
export const prettyNumber = (
  ...
  return [int.replace(/\B(?=(\d{3})+(?!\d))/g, delimiter), frac].filter((e) => !!e).join(dot)
}
...
export const parseNumber = (n: number | string): number => {
  if (typeof n === "string") {
    n = parseFloat(n.replace(/[^\d\.]+/g, "")) / (n.slice(-1) === "%" ? 100 : 1)
  }
  if (n <= Infinity) {
    return n
  } else {
...
export const glyphs = (): Record<string, string> =>
  Object.fromEntries(
    [fromTo(161, 255), fromTo(338, 402), fromTo(8211, 8230), [8240, 8364, 8482]]
      .flat()
 ...
 */
export const glyph = (c: string): string => enclose("&#", c.charCodeAt(0).toString(), ";")
