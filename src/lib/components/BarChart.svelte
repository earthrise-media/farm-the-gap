<script>
  ...
  const fn_color = (o, i, g) => (typeof color === "string" ? color : color(o, i, g))

  let barElements = []

  $: min = data.xAxis?.min || 0
  $: max = groups?.length
    ? Math.max(...data.map((o, i) => groups.map((g, j) => value(o, i, j))).flat())
    : Math.max(...data.map(value))
  $: sortedData = data.sort(sort)
</script>

<div class="bar-chart font-sans" class:axis-lines={showAxisLines} class:grouped={groups?.length}>
    ...
    >
      ...
      {#if groups?.length}
        {#each groups as _, g}
          <div class="w-full relative flex align-center">
            <div
              class="rect"
              style="transform: scale3d({fw(o, i, g)},1,1); height: {barHeight}px; color: {fn_color(
                o,
                i,
                g
              )};"
      ...
      {:else}
        <div class="w-full relative flex align-center">
          <div
            class="rect"
            style="transform: scale3d({fw(o)},1,1); height: {barHeight}px; color: {fn_color(o, i)};"
          />
          <div
            class="value"
            class:opacity-75={!colorLabels}
